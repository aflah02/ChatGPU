...
- If information is not available, explicitly and respectfully state that you do not have the data to answer the question.
...
def clear_chat():
    ...
    st.session_state.model_reply_phase = False
...
if "model_reply_phase" not in st.session_state:
    st.session_state.model_reply_phase = False

if "chat_input" not in st.session_state:
    st.session_state.chat_input = ""

if "populate_default_question_in_chat_input" not in st.session_state:
    st.session_state.populate_default_question_in_chat_input = False
...
with st.sidebar:
    ...
    sample_questions = [
        'Explain the difference between CUDA Cores and Tensor Cores.',
        'ELI5: What is a Kernel?',
        'How does the CUDA programming model work?',
        'What is the Memory Hierarchy?',
        'ELI5: What is Parallel Thread Execution?',
        'What are Registers?',
    ]
...
def send_chat_message():
    st.session_state['model_reply_phase'] = True

if st.session_state['model_reply_phase']:
    user_message = st.session_state['chat_input']
    st.session_state['chat_input'] = ""

if st.session_state['model_reply_phase'] == False:
    if st.session_state['populate_default_question_in_chat_input'] and st.session_state['question_pills']:
        st.session_state['chat_input'] = st.session_state['question_pills']
    prompt = split_cols[0].text_area("", placeholder="Ask ChatGPU", label_visibility="collapsed", key="chat_input")
    split_cols[1].button("", icon=":material/send:", help="Send", on_click=send_chat_message)

    st.session_state['populate_default_question_in_chat_input'] = False

if st.session_state['model_reply_phase']:
    prompt = split_cols[0].text_area("", placeholder="Ask ChatGPU", label_visibility="collapsed", key="chat_input", disabled=True)
    split_cols[1].button("", icon=":material/send:", help="Send", on_click=send_chat_message, disabled=True)
    with chat_container.chat_message("user"):
        st.markdown(user_message)
    st.session_state['messages'].append({"role": "user", "content": user_message})
    with chat_container.chat_message("assistant"):
        response = st.write_stream(make_request(user_message))
